...
const ReposicaoReagentOverlay = ({ onClose }) => {
  ...
  const handleSubmit = async () => {
    if (!selectedReagente) {
      alert("Selecione um reagente para repor o estoque.");
      return;
    }
    ...
    try {
      await atualizarReagente(selectedReagente.id, reagente);
      ...
      alert("Erro ao atualizar reagente. Tente novamente.");
    }
  };
              ...
              <Grid item xs={12} sm={6}>
                <Autocomplete
                  ...
                  onChange={(event, newValue) =>
                    setSelectedTipoReagente(newValue)
                  }
                  renderInput={(params) => (
