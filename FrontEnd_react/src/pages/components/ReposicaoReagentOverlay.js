                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}>
                        <TextField
                            ...
                            value={nome}
                            disabled={Boolean(selectedReagente)}  // Desabilita se reagente selecionado
                    ...
                    <Grid item xs={12} sm={6}>
                        <Autocomplete
                            options={tiposReagentes}  // Lista de tipos de reagentes
                            getOptionLabel={(option) => option.name}
                            value={selectedTipoReagente}
                            onChange={(e, newValue) => setSelectedTipoReagente(newValue)}
                            renderInput={(params) => (
                                <TextField {...params} label="Tipo de Reagente" />
                            )}
                            disabled={Boolean(selectedReagente)}  // Desabilita se reagente selecionado
                        />
                    </Grid>
                    <Grid item xs={12} sm={6}>
                        <Autocomplete
                            options={unidadesReagentes}  // Lista de unidades
                            getOptionLabel={(option) => option.name}
                            value={selectedUnidade}
                            onChange={(e, newValue) => setSelectedUnidade(newValue)}
                            renderInput={(params) => (
                                <TextField {...params} label="Unidade do Reagente" />
                            )}
                            disabled={Boolean(selectedReagente)}  // Desabilita se reagente selecionado
                        />
                    </Grid>
